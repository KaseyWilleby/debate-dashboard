...
function AuthProvider({ children }) {
    ...
    const { isUserLoading: isFirebaseUserLoading, user: firebaseUser } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$firebase$2f$provider$2e$tsx__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useUser"])();
    const { auth } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$firebase$2f$provider$2e$tsx__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useFirebase"])();
    // This is the correct way to get all users, by providing a static list.
    const allUsers = [
        {
            id: '1',
            name: 'Admin Adam',
        ...
        "AuthProvider.useEffect": ()=>{
            ...
            if (storedUser) {
                try {
                    ...
                    // Ensure the user from storage has a Firebase UID
                    const userWithUid = {
                        ...parsedUser,
                        id: firebaseUser?.uid || parsedUser.id
                    };
                    setUser(userWithUid);
                } catch (e) {
    ...
    const login = async (userToLogin)=>{
        const foundUser = allUsers.find((u)=>u.username.toLowerCase() === userToLogin.username.toLowerCase());
        if (foundUser) {
            if (auth && !auth.currentUser) {
                try {
                    ...
                    setUser(userWithUid);
                    localStorage.setItem(CURRENT_USER_STORAGE_KEY, JSON.stringify(userWithUid));
                    router.push('/dashboard/welcome');
                } catch (error) {
                    console.error("Anonymous sign-in failed", error);
                    // Fallback to login without UID if anon sign-in fails
                    setUser(foundUser);
                ...
                // Fallback for when auth is not ready
                setUser(foundUser);
                localStorage.setItem(CURRENT_USER_STORAGE_KEY, JSON.stringify(foundUser));
                router.push('/dashboard/welcome');
            }
        } else {
            throw new Error('User not found. Try one of: admin, varsity, novice, Mr.Willeby');
        }
    };
    const logout = async ()=>{
        if (!auth) return;
...
_s(AuthProvider, "KyWRk5ie0D30MQmj8pBkJ13fkZk=", false, function() {
    return [
...
//# sourceMappingURL=src_1d1167a3._.js.map